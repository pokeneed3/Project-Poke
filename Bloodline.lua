...
local function check(v)
	...
	if v:IsA("NumberValue") then
		notify("number", "Someone Has Chakra Sense")
	elseif v:IsA("Attachment") then
		local parent = v.Parent
		local character = parent and parent.Parent
		if character and Players:GetPlayerFromCharacter(character) then
			notify("attachment", "Someone is spectating you with chakra sense")
		end
	end
end

local function safeCheck(v)
	local ok, err = pcall(check, v)
	if not ok then
		warn("[ChakraSense] " .. tostring(err))
	end
end
...
game.DescendantAdded:Connect(safeCheck)
...
local OriginalFog_End = Lighting.FogEnd
...
VisualMods:AddToggle("Remove_Fog", {
	...
	Callback = function(Value)
		if Value then
			...
			Lighting.FogEnd = OriginalFog_End
		end
	end,
})
...
SaveManager:SetFolder("Project Poke/Bloodlines")
